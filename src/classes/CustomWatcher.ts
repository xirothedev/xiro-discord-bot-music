...
export class CustomWatcher implements QueueChangesWatcher {
    ...
    tracksAdd(guildId: string, tracks: (Track | UnresolvedTrack)[], position: number | number[]) {
        console.log(
            `${this.client.guilds.cache.get(guildId)?.name || guildId}: ${tracks.length} Tracks got added into the Queue at position #${position}`,
        );
    }
    tracksRemoved(guildId: string, tracks: (Track | UnresolvedTrack)[], position: number | number[]) {
        console.log(
