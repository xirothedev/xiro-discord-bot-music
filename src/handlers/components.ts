...
const components = async (client: ExtendedClient) => {
    try {
        const componentTypes = await readdir(`./src/components/`, { withFileTypes: true });

        for (const typeDir of componentTypes) {
            if (!typeDir.isDirectory() || !ComponentsType.includes(typeDir.name)) {
                client.logger.error(`Invalid folder name: ${typeDir.name}, it must be one of ${ComponentsType.join(", ")}`);
                continue;
            }

            const files = await readdir(`./src/components/${typeDir.name}/`);
            const filteredFiles = files.filter(f => f.endsWith(".js") || f.endsWith(".ts"));

            for (const file of filteredFiles) {
                try {
                    const module = (await import(`../components/${typeDir.name}/${file}`)).default;
                    ...
                    client.collection.components[typeDir.name as "selects" | "buttons" | "autocomplete" | "modals"].set(
                        module?.customId || module?.commandName,
                        module
                    );
                } catch (err: any) {
                    client.logger.error(`Error loading component from file ${file}: ${err.message}`);
                }
            }
        }
    } catch (err: any) {
